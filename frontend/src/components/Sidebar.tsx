...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F5F0E8] border-r border-[#E8E2D6] w-72 text-neutral-800 select-none">
      ...
      {/* 3. Middle Scroll Area: Primary Navigation + Recents History List */}
      <div className="flex-1 overflow-y-auto px-3 py-1 space-y-4">
        {/* Navigation Items */}
        <div className="space-y-1">
          <p className="px-2 text-3xs font-extrabold uppercase tracking-wider text-neutral-400 mb-1">Navegación</p>
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              className={({ isActive }) => `
                ...
                ${isActive 
                  ? 'bg-white text-[#9E7111] font-bold shadow-2xs border-[#E8E2D6]' 
                  : 'bg-transparent border-transparent text-neutral-600 hover:bg-white/70 hover:text-neutral-900'}
              `}
            >
              {({ isActive }) => (
                <>
                  <span className={`${isActive ? 'text-[#9E7111]' : 'text-neutral-400'}`}>
                    {item.icon}
                  </span>
        ...
        <div className="space-y-1 pt-2 border-t border-[#E8E2D6]/80">
          ...
          <div className="space-y-0.5">
            {listLoading ? (
              <div className="px-3 py-4 text-center text-xs text-neutral-400 animate-pulse">
                Cargando historial...
              </div>
              ...
              conversations.map((conv) => (
                <div
                  ...
                  className={`
                    group flex items-center justify-between px-3 py-2 rounded-xl text-xs cursor-pointer transition-all duration-150 border
                ...
                >
                  ...
                  <button
                    ...
                    className="p-1 rounded-md text-neutral-400 hover:text-red-600 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all shrink-0"
