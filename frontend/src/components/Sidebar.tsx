    ...
    <div className="flex flex-col h-full bg-[#F5F0E8] border-r border-[#E8E2D6] w-72 text-neutral-800 select-none">
      ...
      <div className="p-3">
        <button
          ...
          `}
          title={isFreshChat ? 'Ya estás en una consulta nueva' : 'Crear una nueva consulta'}
        >
          <div className="flex items-center gap-2.5">
            <span className={`w-5 h-5 rounded-lg flex items-center justify-center font-bold text-sm transition-transform ${isFreshChat ? 'bg-[#E8E2D6]/40 text-neutral-400' : 'bg-[#FAF8F5] border border-[#E8E2D6] text-[#9E7111] group-hover:scale-110'}`}>
              +
            </span>
            <span className={isFreshChat ? 'text-neutral-400 font-semibold' : 'text-neutral-900 font-bold'}>
              Nueva Consulta
            </span>
          </div>
          {!isFreshChat && (
      ...
      {/* 3. Middle Scroll Area: Recents History List */}
